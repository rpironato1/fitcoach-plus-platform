...
export function LocalStorageAuthProvider({
  children,
}: {
    ...
    try {
      ...
      if (profileData) {
        ...
        if (profileData.role === "trainer") {
          const trainerData =
            localStorageService.getTrainerProfileByUserId(userId);
          setTrainerProfile(trainerData);
        } else if (profileData.role === "student") {
          const studentData =
            localStorageService.getStudentProfileByUserId(userId);
  ...
  const signIn = async (email: string, password: string) => {
    try {
      ...
      const isDemoAccount = Object.values(demoCredentials).some(
...
export function useLocalStorageAuth() {
  ...
  if (context === undefined) {
    throw new Error(
      "useLocalStorageAuth must be used within a LocalStorageAuthProvider"
    );
  }
  return context;
}
