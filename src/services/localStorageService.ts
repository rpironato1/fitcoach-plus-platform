...
class LocalStorageService {
   ...
   */
  async signIn(email: string, password: string): Promise<LocalStorageAuthSession> {
    ...
    return {
      ...
      trainer_profiles: [
        {
          ...
          max_students: 40,
          ai_credits: 25,
          active_until: new Date(today.getTime() + 30 * 24 * 60 * 60 * 1000).toISOString(),
          avatar_url: null,
      ...
      notifications: [
        ...
        },
        {
          ...
          message: 'Seu personal trainer criou um novo plano de treino para você',
          type: 'workout',
          read: false,
   ...
   */
  async upgradeTrainerPlan(trainerId: string, newPlan: 'pro' | 'elite'): Promise<void> {
    const data = this.getData();
    if (data) {
      const trainerProfile = data.trainer_profiles.find(tp => tp.id === trainerId);
      if (trainerProfile) {
        trainerProfile.plan = newPlan;
        trainerProfile.max_students = newPlan === 'pro' ? 40 : 100;
        trainerProfile.ai_credits = newPlan === 'pro' ? 50 : 200;
        trainerProfile.active_until = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString();
   ...
   */
  async updateStudentProfile(studentId: string, updates: Partial<LocalStorageStudentProfile>): Promise<void> {
    const data = this.getData();
    if (data) {
      const studentProfile = data.student_profiles.find(sp => sp.id === studentId);
      if (studentProfile) {
        Object.assign(studentProfile, updates, { updated_at: new Date().toISOString() });
        this.setData(data);
      }
    }
  }
  ...
  async quickLoginAsTrainer(): Promise<LocalStorageAuthSession> {
    const trainerUser = this.getUsers().find(u => u.id === this.DEMO_TRAINER_ID);
  ...
  async quickLoginAsStudent(): Promise<LocalStorageAuthSession> {
    const studentUser = this.getUsers().find(u => u.id === this.DEMO_STUDENT_ID);
