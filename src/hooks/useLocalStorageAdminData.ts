...
export function useLocalStorageAdminStats() {
  return useQuery({
    ...
    queryFn: async (): Promise<AdminDashboardStats> => {
      ...
      const averageSessionsPerTrainer = activeTrainers > 0 ? 
        Math.round((completedSessions / activeTrainers) * 10) / 10 : 0;
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<RecentPayment[]> => {
      ...
      const recentPayments = data.payments
        ...
        .slice(0, 10)
        .map(payment => {
          const trainer = data.trainer_profiles.find(t => t.id === payment.trainer_id);
          return {
            ...payment,
            method: 'Cartão de Crédito', // Mock payment method
            trainer_name: trainer ? `Trainer ${payment.trainer_id.slice(-3)}` : 'Desconhecido'
          };
        });
