...
export class LocalStoragePaymentService implements IPaymentService {
  ...
  async cancelSubscription(subscriptionId: string): Promise<void> {
    try {
      ...
      if (subscriptionIndex >= 0) {
        ...
        data.subscriptions[subscriptionIndex].updated_at =
  ...
  async updateTrainerPlan(trainerId: string, plan: TrainerPlan): Promise<void> {
    try {
      ...
      if (trainerIndex >= 0) {
        ...
        data.trainer_profiles[trainerIndex].max_students =
          planLimits.maxStudents;
        data.trainer_profiles[trainerIndex].ai_credits = planLimits.aiCredits;
        data.trainer_profiles[trainerIndex].updated_at =
          new Date().toISOString();
  ...
  async checkPlanLimits(trainerId: string): Promise<boolean> {
    try {
      ...
      const studentCount =
        data.student_profiles?.filter((s) => s.trainer_id === trainerId)
          .length || 0;
