...
export class LocalStorageAIService implements IAIService {
  async generateDietPlan(
    ...
    try {
      ...
      const dietPlan: DietPlan = {
        ...
        target_fat: Math.round((request.target_calories * 0.3) / 9),
  ...
  async getDietPlans(trainerId: string): Promise<DietPlan[]> {
    try {
      const data = localStorageService.getData();
      return (
        data?.diet_plans?.filter((plan) => plan.trainer_id === trainerId) || []
      );
    } catch (error) {
  ...
  async generateWorkoutSuggestion(
    ...
    try {
      ...
      await this.recordAIRequest(
        ...
        "workout_suggestion",
        JSON.stringify(request),
        JSON.stringify(workoutSuggestion),
        creditsRequired
      );
  ...
  async getAIUsageStats(trainerId: string): Promise<AIUsageStats> {
    try {
      const data = localStorageService.getData();
      const requests =
        data?.ai_requests?.filter((req) => req.trainer_id === trainerId) || [];
      const transactions =
        data?.credit_transactions?.filter(
          (tx) => tx.trainer_id === trainerId && tx.type === "usage"
        ) || [];

      const totalRequests = requests.length;
      const creditsUsed = transactions.reduce(
        (sum, tx) => sum + Math.abs(tx.amount),
        0
      );
      const creditsRemaining = await this.getCreditBalance(trainerId);

      const featureCount = requests.reduce(
        (acc, req) => {
      ...
      const mostUsedFeature = Object.keys(featureCount).reduce(
  ...
  async getAIRequests(trainerId: string): Promise<AIRequest[]> {
    try {
      ...
      return (
        data?.ai_requests
          ?.filter((req) => req.trainer_id === trainerId)
          .sort(
            (a, b) =>
  ...
  async deductCredits(
    ...
    try {
      ...
      const trainerIndex = data.trainer_profiles.findIndex(
        (t) => t.id === trainerId
      );
    ...
    try {
      ...
      return (
        data?.credit_transactions
          ?.filter((tx) => tx.trainer_id === trainerId)
          .sort(
            (a, b) =>
              new Date(b.created_at).getTime() -
              new Date(a.created_at).getTime()
    ...
    return [
      {
        ...
        fat: Math.floor((caloriesPerMeal * 0.2) / 9),
        ingredients: [],
      ...
      },
      {
        ...
        meal_type: "lunch",
        calories: caloriesPerMeal,
        protein: Math.floor((caloriesPerMeal * 0.3) / 4),
      ...
      },
      {
        ...
        meal_type: "snack",
        calories: caloriesPerMeal,
        protein: Math.floor((caloriesPerMeal * 0.2) / 4),
        carbs: Math.floor((caloriesPerMeal * 0.6) / 4),
        fat: Math.floor((caloriesPerMeal * 0.2) / 9),
        ingredients: [],
  ...
  private generateMockExercises(
    ...
    for (const group of request.muscle_groups) {
      ...
      exercises.push({
        ...
        instructions:
          "Execute o movimento de forma controlada, mantendo a postura correta.",
      });
    }
