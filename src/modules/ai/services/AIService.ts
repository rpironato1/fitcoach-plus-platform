...
export class OpenAIService implements IAIService {
  ...
  async generateDietPlan(trainerId: string, request: GenerateDietPlanRequest): Promise<DietPlan> {
    try {
      ...
      if (credits < 5) {
        throw new Error('Créditos insuficientes para gerar plano de dieta (necessário: 5 créditos)');
      }
      ...
      const dietPlan = this.parseDietPlanResponse(aiResponse, trainerId, request.student_id);
  ...
  async generateWorkoutSuggestion(trainerId: string, request: GenerateWorkoutRequest): Promise<WorkoutSuggestion> {
    try {
      ...
      const credits = await this.getCreditBalance(trainerId);
      if (credits < 3) {
        throw new Error('Créditos insuficientes para gerar sugestão de treino (necessário: 3 créditos)');
      }
      ...
      // Record AI request and deduct credits
      await this.recordAIRequest(trainerId, 'workout_suggestion', prompt, aiResponse, 3);
  ...
  async getAIUsageStats(trainerId: string): Promise<AIUsageStats> {
    ...
    const requests = data || [];
    const totalCreditsUsed = requests.reduce((sum, req) => sum + req.cost_credits, 0);
    const successfulRequests = requests.filter(req => req.response && req.response.length > 0);
    ...
    return {
      ...
      success_rate: requests.length > 0 ? (successfulRequests.length / requests.length) * 100 : 0,
    };
  }
  ...
  private parseDietPlanResponse(response: string, trainerId: string, studentId: string): Omit<DietPlan, 'id'> {
  ...
  private getMockDietPlan(trainerId: string, studentId: string): DietPlan {
    return {
      ...
      description: 'Plano focado em emagrecimento saudável com refeições equilibradas',
