...
export class LocalStorageWorkoutService implements IWorkoutService {
  ...
  async getExercises(trainerId?: string): Promise<Exercise[]> {
    try {
      ...
      return data.exercises.filter(exercise => 
        exercise.is_public || exercise.trainer_id === trainerId
  ...
  private enrichWorkoutPlanWithExercises(plan: WorkoutPlan, data: any): WorkoutPlan {
    if (!data?.workout_plan_exercises || !data?.exercises) {
      return plan;
    }
    
    const planExercises = data.workout_plan_exercises
      .filter((pe: any) => pe.workout_plan_id === plan.id)
      .map((pe: any) => {
        const exercise = data.exercises.find((e: Exercise) => e.id === pe.exercise_id);
        return {
          ...pe,
          exercise: exercise || {
            id: pe.exercise_id,
