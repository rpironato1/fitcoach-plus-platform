...
export class LocalStorageSecurityService implements ISecurityService {
  ...
  async updatePrivacySettings(
    ...
    try {
      ...
      const existingIndex = data.privacy_settings.findIndex(
        (s) => s.user_id === userId
      );
      ...
      const privacySettings: PrivacySettings = {
        id:
          existingIndex >= 0
            ? data.privacy_settings[existingIndex].id
            : `privacy_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        user_id: userId,
  ...
  async getDataRequests(userId: string): Promise<{
    ...
    try {
      ...
      const exports =
        data?.data_export_requests?.filter((r) => r.user_id === userId) || [];
      const deletions =
        data?.data_deletion_requests?.filter((r) => r.user_id === userId) || [];
  ...
  async logAuditEvent(
    ...
    try {
      ...
      if (data) {
        ...
        const userLogs = data.audit_logs.filter(
          (log) => log.user_id === event.user_id
        );
  ...
  async generateComplianceReport(
    ...
    try {
      ...
      const dataRequests =
        ...
        (data?.data_deletion_requests?.length || 0);
      const securityIncidents =
        data?.security_alerts?.filter(
          (alert) => alert.severity === "high" || alert.severity === "critical"
        ).length || 0;

      const consentedUsers = new Set(
        consents.filter((c) => c.consented).map((c) => c.user_id)
      ).size;
      const consentRate =
        totalUsers > 0 ? (consentedUsers / totalUsers) * 100 : 100;
      ...
      const report: ComplianceReport = {
        ...
        details: {
          consents_by_type: consents.reduce(
            (acc, consent) => {
