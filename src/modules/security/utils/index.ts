...
export function isConsentValid(
  ...
  const expiryDate = new Date(
    consentDate.getTime() + validityDays * 24 * 60 * 60 * 1000
  );
...
export function getPrivacyRecommendations(
  ...
  if (settings.profile_visibility === "public") {
    recommendations.push(
      "Considere tornar seu perfil privado para maior segurança"
    );
  }

  if (settings.data_retention_days > 1095) {
    // 3 years
  ...
  if (!settings.analytics_consent) {
    recommendations.push(
      "Habilite analytics para ajudar a melhorar nossos serviços"
    );
  }
  ...
  return [
    {
      ...
      description:
        "Autorização para processar seus dados pessoais conforme nossa política",
      required: true,
    },
    {
      ...
      description:
        "Permissão para coletar dados analíticos para melhorar nossos serviços",
      required: true,
    },
    {
 ...
 */
export function getDataRetentionPeriods(): Array<{
  value: number;
  label: string;
  description: string;
}> {
