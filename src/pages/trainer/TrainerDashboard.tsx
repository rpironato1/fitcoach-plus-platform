...
export default function TrainerDashboard() {
  ...
  const { data: supabaseStats, isLoading: supabaseStatsLoading } =
    useDashboardStats();
  const { data: supabaseSessions, isLoading: supabaseSessionsLoading } =
    useUpcomingSessions();
  const { data: supabaseActivity, isLoading: supabaseActivityLoading } =
    useRecentActivity();

  const { data: localStats, isLoading: localStatsLoading } =
    useLocalStorageDashboardStats();
  const { data: localSessions, isLoading: localSessionsLoading } =
    useLocalStorageUpcomingSessions();
  ...
  const sessionsLoading = useLocalStorage
    ? localSessionsLoading
    : supabaseSessionsLoading;
  const activityLoading = useLocalStorage
    ? localActivityLoading
    : supabaseActivityLoading;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              R${" "}
              {(stats?.monthlyRevenue || 0).toLocaleString("pt-BR", {
