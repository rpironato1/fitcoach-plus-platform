...
export default function ReportsPage() {
  const { data: reportData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [trainersResult, studentsResult, paymentsResult, sessionsResult] =
        await Promise.all([
      ...
      const monthlyRevenue =
        paymentsResult.data
          ...
          .reduce(
            (acc, payment) => {
              const month = new Date(payment.created_at).toLocaleString(
                "pt-BR",
                { month: "short" }
              );
      ...
      return {
        ...
        planDistribution: Object.entries(planDistribution).map(
          ([plan, count]) => ({
            name: plan,
            value: count,
          })
