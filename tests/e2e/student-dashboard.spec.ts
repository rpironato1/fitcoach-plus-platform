...
test.describe('FitCoach Student Dashboard E2E Tests', () => {
  ...
  test('should display student dashboard with personal information', async ({ page }) => {
    await loginWithRole(page, 'student');
    await page.goto('/student');
    await page.waitForLoadState('networkidle');
  ...
  test('should test menstrual cycle feature activation', async ({ page }) => {
    ...
    const profileLink = page.getByRole('link', { name: 'Perfil' }).or(
      page.getByRole('button', { name: 'Perfil' }).or(
        page.locator('text=Informações Pessoais').first()
      )
    );
      ...
      for (const selector of menstrualElements) {
        ...
        if (await element.isVisible({ timeout: 5000 })) {
          ...
          try {
            ...
            console.log('🌸 Menstrual cycle adaptive features active:', foundAdaptiveContent);
          } catch (error) {
  ...
  test('should display workout plans and tracking', async ({ page }) => {
    ...
    const workoutsLink = page.getByRole('link', { name: 'Treinos' }).or(
      page.getByRole('button', { name: 'Treinos' }).or(
        page.locator('text=Workout, text=Exercícios').first()
      )
    );
    ...
    expect(foundWorkouts || await page.locator('text=Nenhum treino, text=Sem plano').isVisible()).toBeTruthy();
  });
  ...
  test('should display nutrition plans and tracking', async ({ page }) => {
    ...
    const nutritionLink = page.getByRole('link', { name: 'Nutrição' }).or(
      page.getByRole('button', { name: 'Nutrição' }).or(
        page.locator('text=Dieta, text=Alimentação').first()
      )
    );
    ...
    expect(foundNutrition || await page.locator('text=Nenhuma dieta, text=Sem plano').isVisible()).toBeTruthy();
  });
  ...
  test('should show session history and schedule', async ({ page }) => {
    ...
    const sessionsLink = page.getByRole('link', { name: 'Sessões' }).or(
      page.getByRole('button', { name: 'Sessões' }).or(
        page.locator('text=Histórico, text=Agendamentos').first()
      )
    );
  ...
  test('should handle student logout', async ({ page }) => {
    ...
    for (const selector of logoutElements) {
      ...
      if (await element.isVisible({ timeout: 3000 })) {
        ...
        const isOnLanding = page.url().includes('/') && !page.url().includes('/student');
...
test.describe('FitCoach Student UI Login Tests', () => {
  ...
  test('should login student through UI modal', async ({ page }) => {
    ...
    // Login through UI
    await loginThroughUI(page, TEST_CREDENTIALS.student.email, TEST_CREDENTIALS.student.password);
...
test.describe('FitCoach Menstrual Cycle Feature Tests', () => {
  ...
  test('should test complete menstrual cycle feature workflow', async ({ page }) => {
    await loginWithRole(page, 'student');
    await page.goto('/student-demo');
    await page.waitForLoadState('networkidle');
    ...
    console.log(`🌸 Menstrual cycle feature elements found: ${foundFeatureElements}/6`);
    console.log(`🎯 Adaptive content elements found: ${foundAdaptiveElements}/6`);
