...
async function setupLocalStorageAuth(
  page: Page,
...
async function analyzePage(page: Page, userType: string) {
  ...
  try {
    ...
    if (loadingElements > 0) {
      issues.push(
        `Found ${loadingElements} loading element(s) - might be stuck`
      );
    }
...
test.describe("Dashboard Screenshots and Analysis", () => {
  ...
  test("Admin Dashboard - Screenshot and Analysis", async ({ page }) => {
    ...
    await page.waitForSelector(
      '[data-testid="admin-dashboard"], .admin-dashboard, h1',
      { timeout: 10000 }
    );
  ...
  test("Trainer Dashboard - Screenshot and Analysis", async ({ page }) => {
    ...
    // Wait for dashboard to load
    await page.waitForSelector(
      '[data-testid="trainer-dashboard"], .trainer-dashboard, h1',
      { timeout: 10000 }
    );
  ...
  test("Student Dashboard - Screenshot and Analysis", async ({ page }) => {
    ...
    // Wait for dashboard to load
    await page.waitForSelector(
      '[data-testid="student-dashboard"], .student-dashboard, h1',
      { timeout: 10000 }
    );
  ...
  test("Comprehensive Navigation Test", async ({ page }) => {
    ...
    for (const userType of ["admin", "trainer", "student"] as const) {
      ...
      const navScreenshot = await captureFullPageScreenshot(
        page,
        `${userType}-navigation`
      );
    ...
    testResults.forEach((result) => {
      ...
      result.navigationLinks.forEach((link: string) =>
        console.log(`  - ${link}`)
      );
    });
