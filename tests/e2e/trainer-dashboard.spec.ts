...
test.describe("FitCoach Trainer Dashboard E2E Tests", () => {
  ...
  test("should display trainer dashboard overview with metrics", async ({
    page,
  }) => {
    await loginWithRole(page, "trainer");
    await page.goto("/trainer");
  ...
  test("should display students list and management", async ({ page }) => {
    ...
    const studentsLink = page
      ...
      .or(page.getByRole("button", { name: "Alunos" }));
    ...
    expect(
      foundStudents ||
        (await page
          .locator("text=Nenhum aluno, text=Adicionar aluno")
          .isVisible())
    ).toBeTruthy();
  });
  ...
  test("should handle session scheduling", async ({ page }) => {
    ...
    const sessionsLink = page
      ...
      .or(page.getByRole("button", { name: "Sessões" }));
  ...
  test("should display and manage workout plans", async ({ page }) => {
    ...
    const plansLink = page
      ...
      .or(page.getByRole("button", { name: "Planos" }));
    ...
    if (await plansLink.isVisible({ timeout: 5000 })) {
      ...
      expect(
        foundPlans || (await page.locator("text=Nenhum plano").isVisible())
      ).toBeTruthy();
    }
  });
  ...
  test("should show payment tracking", async ({ page }) => {
    ...
    const paymentsLink = page
      ...
      .or(page.getByRole("button", { name: "Pagamentos" }));
    ...
    if (await paymentsLink.isVisible({ timeout: 5000 })) {
      ...
      expect(
        foundPayments ||
          (await page.locator("text=Nenhum pagamento").isVisible())
      ).toBeTruthy();
    }
  });
  ...
  test("should display trainer profile and settings", async ({ page }) => {
    ...
    const profileLink = page
      ...
      .or(page.getByRole("button", { name: "Perfil" }));
  ...
  test("should handle trainer logout", async ({ page }) => {
    ...
    for (const selector of logoutElements) {
      ...
      if (await element.isVisible({ timeout: 3000 })) {
        ...
        const isOnLanding =
          page.url().includes("/") && !page.url().includes("/trainer");
