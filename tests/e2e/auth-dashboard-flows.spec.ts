...
async function setupTestData(page: Page) {
  ...
  await page.evaluate(() => {
    ...
    localStorage.setItem('fitcoach_trainer_profiles', JSON.stringify(trainerProfiles));
    localStorage.setItem('fitcoach_student_profiles', JSON.stringify(studentProfiles));
  });
}

async function performLogin(page: Page, userType: 'admin' | 'trainer' | 'student') {
  ...
  console.log(`Login form visible: ${loginFormVisible}, Login button: ${hasLoginButton}`);
...
test.describe('Authentication Flows', () => {
  test('should test admin login and dashboard access', async ({ page }) => {
    ...
    console.log(`Has admin content: ${hasAdminContent}, Has trainers text: ${hasTrainersText}`);
    ...
    const sessionData = await page.evaluate(() => {
      return {
        ...
        keys: Object.keys(localStorage).filter(k => k.startsWith('fitcoach_'))
      };
    });
  ...
  test('should test trainer login and dashboard access', async ({ page }) => {
    ...
    const hasTrainerContent = await page.locator('text=Alunos').isVisible() || 
  ...
  test('should test student login and dashboard access', async ({ page }) => {
    ...
    const hasStudentContent = await page.locator('text=Treino').isVisible() || 
...
test.describe('Dashboard Navigation Tests', () => {
  test('should test demo dashboard functionality', async ({ page }) => {
    ...
    if (buttons > 0) {
      ...
      if (buttonText && !buttonText.includes('Sair') && !buttonText.includes('Logout')) {
  ...
  test('should test localStorage manager functionality', async ({ page }) => {
    ...
    const hasDataModeControls = await page.locator('text=Modo').isVisible() ||
...
test.describe('Route Protection and Navigation', () => {
  test('should verify route protection without authentication', async ({ page }) => {
    ...
    for (const route of protectedRoutes) {
      ...
      await takeScreenshot(page, `route-protection-${route.replace(/\//g, '-')}`);
    }
  });

  test('should verify error handling for non-existent routes', async ({ page }) => {
    const invalidRoutes = [
      '/non-existent',
      '/invalid-page',
      '/admin/non-existent',
