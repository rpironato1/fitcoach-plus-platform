...
test.describe('FitCoach Complete User Flow E2E Tests', () => {
  ...
  test('should complete full user journey from landing to subscription', async ({ page }) => {
    ...
    const choosePlanButton = page.getByRole('button', { name: 'Escolher Plano' }).first();
    const startFreeButton = page.getByRole('button', { name: 'Começar Grátis' }).first();
    ...
    const hasModal = await page.getByRole('dialog').isVisible({ timeout: 3000 });
  ...
  test('should test cross-role navigation and access control', async ({ page }) => {
    ...
    console.log(`🔒 Student access control working: ${isRestrictedFromStudent}`);
  ...
  test('should test localStorage persistence across page reloads', async ({ page }) => {
    // Login as admin
  ...
  test('should test complete subscription flow simulation', async ({ page }) => {
    ...
    for (const tier of pricingTiers) {
      ...
      const choosePlanBtn = tierSection.getByRole('button', { name: /Escolher|Começar|Upgrade/i }).first();
  ...
  test('should test all interactive elements and CTAs', async ({ page }) => {
    ...
    const tabs = ['Gestão', 'Agendamento', 'IA', 'Sistema', 'Analytics', 'Mobile'];
    ...
    // Test FAQ accordion
    const faqButtons = page.getByRole('button', { name: /Como funciona|Quanto custa|Posso cancelar/ });
  ...
  test('should test complete accessibility compliance across all pages', async ({ page }) => {
    const pagesToTest = [
  ...
  test('should test complete responsive design across all viewports', async ({ page }) => {
    const pagesToTest = [
      { url: '/', name: 'Landing Page' },
      { url: '/localStorage-manager', name: 'LocalStorage Manager' }
    ];
  ...
  test('should verify localStorage data integrity and export functionality', async ({ page }) => {
    await setupTestEnvironment(page);
    ...
    const dataStructure = await page.evaluate(() => {
      ...
      return {
        ...
        platformDataValid: platformData ? JSON.parse(platformData).users !== undefined : false
      };
    });
  ...
  test('should test error handling and edge cases', async ({ page }) => {
    ...
    const pageLoaded = await page.getByRole('heading').first().isVisible({ timeout: 5000 });
