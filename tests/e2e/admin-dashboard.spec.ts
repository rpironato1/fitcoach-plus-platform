...
test.describe('FitCoach Admin Dashboard E2E Tests', () => {
  ...
  test('should display admin dashboard overview with metrics', async ({ page }) => {
    await loginWithRole(page, 'admin');
    await page.goto('/admin');
    await page.waitForLoadState('networkidle');
    ...
    for (const item of navItems) {
      const navElement = page.getByRole('link', { name: item }).or(
        page.getByRole('button', { name: item }).or(
  ...
  test('should navigate between admin sections', async ({ page }) => {
    ...
    const usersLink = page.getByRole('link', { name: 'Usuários' }).or(
      page.getByRole('button', { name: 'Usuários' })
    );
    ...
    if (await usersLink.isVisible({ timeout: 5000 })) {
      ...
      const usersContent = page.locator('table, [data-testid="users-list"], text=Lista de Usuários').first();
      await expect(usersContent).toBeVisible({ timeout: 10000 });
    }
    
    // Test navigation to trainers section
    const trainersLink = page.getByRole('link', { name: 'Trainers' }).or(
      page.getByRole('button', { name: 'Trainers' })
    );
    ...
    if (await trainersLink.isVisible({ timeout: 5000 })) {
      ...
      const trainersContent = page.locator('table, [data-testid="trainers-list"], text=Personal Trainers').first();
  ...
  test('should handle user management functionality', async ({ page }) => {
    ...
    const usersSection = page.getByRole('link', { name: 'Usuários' }).or(
      page.getByRole('button', { name: 'Usuários' }).or(
        page.locator('text=Usuários').first()
      )
    );
...
test.describe('FitCoach Admin UI Login Tests', () => {
  ...
  test('should login admin through UI modal', async ({ page }) => {
    ...
    // Login through UI
    await loginThroughUI(page, TEST_CREDENTIALS.admin.email, TEST_CREDENTIALS.admin.password);
  ...
  test('should handle invalid admin credentials', async ({ page }) => {
    ...
    const hasErrorMessage = await page.locator('text=erro, text=inválido, text=incorreto').isVisible({ timeout: 3000 });
