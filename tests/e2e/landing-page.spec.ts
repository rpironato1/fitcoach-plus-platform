...
test.describe("FitCoach Landing Page E2E Tests", () => {
  ...
  test("should display landing page with all sections", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", {
        name: /A plataforma mais completa para Personal Trainers/,
      })
    ).toBeVisible();
  ...
  test("should have working tabs functionality", async ({ page }) => {
    ...
    await expect(page.getByRole("tab", { name: "Gestão" })).toHaveAttribute(
      "aria-selected",
      "true"
    );
    ...
    await expect(page.getByRole("tab", { name: "Sistema" })).toHaveAttribute(
      "aria-selected",
      "true"
    );
  ...
  test("should have working FAQ accordion", async ({ page }) => {
    ...
    const faqButton = page.getByRole("button", {
      name: "Como funciona o período gratuito?",
    });
    await faqButton.click();
    await expect(faqButton).toHaveAttribute("aria-expanded", "true");
    await expect(
      page.getByText("Você pode usar o plano Free indefinidamente")
    ).toBeVisible();
  ...
  test("should open login dialog", async ({ page }) => {
    ...
    await expect(
      page.getByRole("dialog", { name: "Bem-vindo de volta!" })
    ).toBeVisible();
  ...
  test("should have working pricing section", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: "Escolher Plano" })
    ).toBeVisible();
  });
  ...
  test("should handle button clicks gracefully", async ({ page }) => {
    ...
    const experimentarButton = page
      .getByRole("button", { name: "Experimentar Agora" })
      .first();
    ...
    const começarGratisButtons = page.getByRole("button", {
      name: "Começar Grátis",
    });
  ...
  test("should have complete footer content", async ({ page }) => {
    ...
    await expect(
      page.getByRole("link", { name: "Central de Ajuda" })
    ).toBeVisible();

    // Test footer copyright
    await expect(
      page.getByText("© 2024 FitCoach. Todos os direitos reservados.")
    ).toBeVisible();
  });
});
...
test.describe("FitCoach Navigation Tests", () => {
  ...
  test("should handle 404 routes", async ({ page }) => {
    ...
    const isOn404 =
      page.url().includes("/nonexistent-route") || page.url().includes("/");
