...
export async function verifyDashboardAccess(
  page: Page,
  expectedRole: "admin" | "trainer" | "student"
) {
  ...
  const dashboardSelectors = {
    admin:
      '[data-testid="admin-dashboard"], h1:has-text("Admin"), h1:has-text("Painel")',
    trainer:
      '[data-testid="trainer-dashboard"], h1:has-text("Trainer"), h1:has-text("Personal")',
    student:
      '[data-testid="student-dashboard"], h1:has-text("Student"), h1:has-text("Aluno")',
  };
...
export async function testResponsiveDesign(page: Page) {
  ...
  for (const viewport of viewports) {
    ...
    console.log(
      `✅ ${viewport.name} (${viewport.width}x${viewport.height}) layout verified`
    );
  }
